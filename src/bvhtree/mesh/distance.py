...
def closest_point_on_line(point: np.ndarray, line_vertices: np.ndarray) -> np.ndarray:
    # Edge vectors
...
def closest_point_on_box(point: np.ndarray, box_min: np.ndarray, box_max: np.ndarray) -> np.ndarray:
    ...
    # Find the closest point inside a box
    ...
    # Outside the box
    return np.maximum(box_min, np.minimum(point, box_max))
